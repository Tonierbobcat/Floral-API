...
public class PlayerAttributeManager extends ProfileComponent implements Mappable<Object> {

    private final AttributeMap map;

    private int currentAttributePoints;

    public PlayerAttributeManager(ProfileData profile) {
        super(profile);
    ...
    public PlayerAttributeManager(ProfileData profile, Map<String,Object> data) {
        super(profile);
        this.map = new AttributeMap(profile);
        if (data != null) {
    ...
    private Map<String, Integer> getAttriubteMap() {
        ...
        for (Attribute attribute : Attribute.values()) {
            ...
            if (instance == null)
                continue;;
            result.put(attribute.name(), instance.getBase());
        }
        return result;
    }
